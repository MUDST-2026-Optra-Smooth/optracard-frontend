...
export function SPAD_Staff() {
  ...
  return <SPAD_Shell><div className="mx-auto max-w-[1440px] space-y-4">
    <SPAD_SectionHeading title="Manage Staff" description="Administrative accounts with access to the management workspaces." action={<><Link to="/superadmin/users" className="rounded border border-[#dfe4eb] bg-white px-3 py-2 text-[9px] font-semibold text-[#687486]">View users</Link>{staff && <SPAD_ExportButton label="Export staff" fileName="optracard-staff.csv" rows={visible.map((member) => ({ id: member.id, username: member.username, email: member.email, role: member.role, phone: member.phone ?? '', created_at: member.createdAt ?? '' }))} />}<Link to="/superadmin/staff/add" className="rounded bg-[#2f65ff] px-3 py-2 text-[9px] font-semibold text-white hover:bg-[#1647c4]">+ Add admin</Link></>} />
