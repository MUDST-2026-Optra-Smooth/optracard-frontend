...
export const StartSelling = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f9fafb] font-sans pb-20"><div className="max-w-5xl mx-auto px-6 pt-10">
      <button type="button" onClick={() => currentStep === 1 ? navigate(-1) : setCurrentStep((step) => step - 1)} className="text-sm font-bold text-black mb-4 hover:underline cursor-pointer">← Back</button>
      ...
      <form onSubmit={handleNext} className="max-w-xl mx-auto mt-20">
        ...
        <div className="flex gap-4 mt-8">{currentStep > 1 && <button type="button" onClick={() => setCurrentStep((step) => step - 1)} className="w-1/2 border border-black text-black py-3.5 rounded-full cursor-pointer">Previous</button>}<button type="submit" disabled={isSaving} className={`${currentStep > 1 ? 'w-1/2' : 'w-full'} bg-[#0b0f19] disabled:opacity-50 text-white py-3.5 rounded-full font-medium cursor-pointer`}>{isSaving ? 'Saving…' : currentStep === 4 ? 'Submit for approval' : 'Next'}</button></div>
      </form>
    </div></div>
  );
};
