...
export function SPAD_AddAdmin() {
  ...
  return <SPAD_Shell><div className="mx-auto max-w-2xl space-y-4">
    <SPAD_SectionHeading title="Add administrator" description="Create a real ADMIN account for the management workspace." action={<Link to="/superadmin/staff" className="rounded border border-[#dfe4eb] bg-white px-3 py-2 text-[9px] font-semibold text-[#687486]">← Back to staff</Link>} />
    <SPAD_Panel title="Administrator account" subtitle="This account receives the ADMIN role. Only a protected database change can create a Super Admin.">
      <form onSubmit={(event) => void submit(event)} className="space-y-4 px-5 py-5">
        ...
        <div className="flex justify-end gap-2 border-t border-[#edf0f4] pt-4"><Link to="/superadmin/staff" className="rounded border border-[#dfe4eb] px-4 py-2 text-[9px] font-semibold text-[#687486]">Cancel</Link><button disabled={saving} type="submit" className="rounded bg-[#2f65ff] px-4 py-2 text-[9px] font-semibold text-white disabled:cursor-not-allowed disabled:opacity-60">{saving ? 'Creating…' : 'Create administrator'}</button></div>
      </form>
    </SPAD_Panel>
  </div></SPAD_Shell>;
}
