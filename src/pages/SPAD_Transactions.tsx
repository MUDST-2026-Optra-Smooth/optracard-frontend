...
export function SPAD_Transactions() {
  ...
  return <SPAD_Shell><div className="mx-auto max-w-[1440px] space-y-4">
    <SPAD_SectionHeading
      ...
      description="Orders recorded in the platform database, grouped by their stored seller and fulfillment status."
      action={
        <div className="flex items-center gap-2">
          <button
            ...
            title="Refresh transactions from live database"
          >
            <RefreshCcw className="h-3 w-3 text-[#2f65ff]" />
            Refresh
          </button>
          {transactions && <SPAD_ExportButton label="Export transactions" fileName="optracard-transactions.csv" rows={visible.map((transaction) => ({ order: transaction.orderNumber, created_at: transaction.createdAt ?? '', buyer: transaction.buyerName, store: transaction.storeName ?? 'Optracard Official Store', source: transaction.source, total: transaction.total ?? 0, status: transaction.status, payment_status: transaction.paymentStatus ?? '' }))} />}
        </div>
      }
    />
    {error ? <SPAD_Error message={error} onRetry={() => void load()} /> : !transactions ? <SPAD_Loading /> : <>
      ...
      <SPAD_FilterBar><SPAD_FilterField label="Search order"><SPAD_Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Order number, buyer, store, or source" /></SPAD_FilterField><SPAD_FilterField label="Fulfillment status"><select value={status} onChange={(event) => setStatus(event.target.value)} className="h-8 w-full cursor-pointer rounded border border-[#dfe4eb] bg-white px-2.5 text-[9px] text-[#687486]"><option value="ALL">All statuses</option><option value="PROCESSING">Processing</option><option value="SHIPPED">Shipped</option><option value="DELIVERED">Delivered</option><option value="CANCELED">Canceled</option></select></SPAD_FilterField><div className="self-end text-[9px] text-[#8e99aa]">{visible.length} matching orders</div></SPAD_FilterBar>
