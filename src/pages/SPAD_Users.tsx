...
export function SPAD_Users() {
  ...
  return <SPAD_Shell><div className="mx-auto max-w-[1440px] space-y-4">
    <SPAD_SectionHeading
      title="Manage User"
      description="Customer and seller accounts from the platform database. Staff accounts are managed separately."
      action={
        <div className="flex items-center gap-2">
          <button
            ...
            title="Refresh users from live database"
          >
            <RefreshCcw className="h-3 w-3 text-[#2f65ff]" />
            Refresh
          </button>
          {users && <SPAD_ExportButton label="Export users" fileName="optracard-users.csv" rows={visible.map((user) => ({ id: user.id, username: user.username, email: user.email, role: user.role, phone: user.phone ?? '', store: user.storeName ?? '', store_status: user.storeStatus ?? '', joined_at: user.createdAt ?? '' }))} />}
        </div>
      }
    />
